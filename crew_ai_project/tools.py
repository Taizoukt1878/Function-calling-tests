...
class OfficialTweetsTool(BaseTool):
    ...
    description : str = "Ftech the latets tweets from the official account of a given username."

    def _run(self, username: str):
        logger.info(f"Fetching official tweets for {username}")
        url = (
            f"http://backend-277369611639.us-central1.run.app/get_official_tweets?username={username}"
        )
...
class GetCategoryMarketDataTool(BaseTool):
    ...
    def _run(self, category: str):
        ...
        url = (
            f"https://us-central1-third-opus-411016.cloudfunctions.net/SearchEngineApiV4/category_market_data?query={category}"
        )
        response = requests.get(url)
